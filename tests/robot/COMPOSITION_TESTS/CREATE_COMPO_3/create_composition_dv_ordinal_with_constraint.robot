...
...             ${\n}Based on:
...             https://github.com/ehrbase/ehrbase/blob/develop/doc/conformance_testing/COMPOSITION_VALIDATION_DATATYPES.md#322-test-case-dv_ordinal-with-constraints
...
Composition With DV_ORDINAL Symbol local::at0006 And Value 1 With Constraints
    ...
    ...     - ${symbolValue}, ${dvOrdinalValue} arguments values local::at0006, 1
    ...     - commit composition
    ...     - check status code of the commited composition.
    ...     - Expected status code on commit composition = 422.
    Set Suite Variable      ${composition_file}    Test_dv_ordinal_with_constraints.v0__.json
    Set Suite Variable      ${optFile}             all_types/Test_dv_ordinal_with_constraints.v0.opt
    Precondition
    ${expectedStatusCode}   Set Variable    422
    ${statusCodeBoolean}    Commit Composition With Modified DV_ORDINAL Symbol And Value
    ...     local::at0006     1     ${expectedStatusCode}
...
Composition With DV_ORDINAL Symbol local::at0005 And Value 2 With Constraints
    ...
    ...     - load json file from CANONICAL_JSON folder
    ...     - update DV_ORDINAL Symbol And Value using:
    ...     - ${symbolValue}, ${dvOrdinalValue} arguments values local::at0005, 2
    ...     - commit composition
    ...     - check status code of the commited composition.
    ...     - Expected status code on commit composition = 422.
    ${expectedStatusCode}   Set Variable    422
    ${statusCodeBoolean}    Commit Composition With Modified DV_ORDINAL Symbol And Value
    ...     local::at0005     2     ${expectedStatusCode}
    IF      ${statusCodeBoolean} == ${FALSE}
        Fail    Commit composition expected status code ${expectedStatusCode} is different.
    END
    [Teardown]  Delete Composition Using API
...
Composition With DV_ORDINAL Symbol local::at0006 And Value 2 With Constraints
    ...
    ...     - load json file from CANONICAL_JSON folder
    ...     - update DV_ORDINAL Symbol And Value using:
    ...     - ${symbolValue}, ${dvOrdinalValue} arguments values local::at0006, 2
    ...     - commit composition
    ...     - check status code of the commited composition.
    ...     - Expected status code on commit composition = 422.
    ${expectedStatusCode}   Set Variable    422
    ${statusCodeBoolean}    Commit Composition With Modified DV_ORDINAL Symbol And Value
    ...     local::at0006     2     ${expectedStatusCode}
    IF      ${statusCodeBoolean} == ${FALSE}
        Fail    Commit composition expected status code ${expectedStatusCode} is different.
    END
    [Teardown]  Delete Composition Using API

Composition With DV_ORDINAL Symbol local::at0005 And Value 1 With Constraints
    [Tags]      Positive
    [Documentation]     *Test case DV_ORDINAL Symbol local::at0005 And Value 1 With Constraints:*
    ...     - load json file from CANONICAL_JSON folder
    ...     - update DV_ORDINAL Symbol And Value using:
    ...     - ${symbolValue}, ${dvOrdinalValue} arguments values local::at0005, 1
    ...     - commit composition
    ...     - check status code of the commited composition.
    ...     - Expected status code on commit composition = 201.
    ${expectedStatusCode}   Set Variable    201
    ${statusCodeBoolean}    Commit Composition With Modified DV_ORDINAL Symbol And Value
    ...     local::at0005     1     ${expectedStatusCode}
    IF      ${statusCodeBoolean} == ${FALSE}
        Fail    Commit composition expected status code ${expectedStatusCode} is different.
    END
...
Change Json KeyValue and Save Back To File
    ...
    ...     content[0].data.events[0].data.items[0].value.symbol.value
    ${symbolValueJsonPath2}     Set Variable
    ...     content[0].data.events[1].data.items[0].value.symbol.value
    ${symbolValueJsonPath3}     Set Variable
    ...     content[0].data.events[2].data.items[0].value.symbol.value
