...
*** Test Cases ***
Test Distinct: SELECT e/erh_id/value AS full FROM EHR e contains COMPOSITIONS C
    [Documentation]     - *Precondition:* 1. Create OPT; 2. Create EHR; 3. Create 2x Compositions
    ...         - Send AQL 'SELECT e/erh_id/value AS full FROM EHR e contains COMPOSITIONS C'
    ...         - Check that result contains 2 rows
    ...         - Check if actual response == expected response
    #[Tags]      not-ready
    [Setup]     Precondition
    ${query1}    Set Variable    SELECT e/erh_id/value AS full FROM EHR e contains COMPOSITIONS C
...
Test Distinct: SELECT DISTINCT e/erh_id/value AS full FROM EHR e contains COMPOSITIONS C
    [Documentation]     \n
    ...         - Send AQL 'SELECT DISTINCT e/erh_id/value AS full FROM EHR e contains COMPOSITIONS C'
    ...         - Check that result contains 1 row
    ...         - Check if actual response == expected response
    ...         - *Postcondition:* Delete EHR using ADMIN endpoint. This is deleting compositions linked to EHR.
    #[Tags]      not-ready
    ${query2}    Set Variable    SELECT DISTINCT e/erh_id/value AS full FROM EHR e contains COMPOSITIONS C
